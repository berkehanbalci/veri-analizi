# Veri Analizi — Ülkeler, Performans ve ETL

Pandas ile veri okuma, temizleme, analiz ve görselleştirme çalışması; SQLite üzerinde büyük veriyle index performansı ölçümü; PostgreSQL entegrasyonu; dış API'lerden canlı veri çekme; ve bunların hepsini birleştiren bir ETL (Extract-Transform-Load) pipeline'ı.
...
- requests (dış API'lere istek atma)
...
├── api_veri_cek.py           # Geocoding + hava durumu API'lerinden canlı veri çekme
├── etl_pipeline.py           # Tam ETL akışı: extract → transform → load
├── hava_durumu.csv           # api_veri_cek.py çıktısı (anlık ölçüm)
...
```
pip install pandas matplotlib sqlalchemy psycopg2-binary python-dotenv requests
```
...
### 6. Dış API'lerden Canlı Veri Çekme (`api_veri_cek.py`)
Şu ana kadarki tüm veriler önceden hazırlanmış dosyalardı. Bu adımda, iki farklı Open-Meteo API'si zincirlenerek **gerçek zamanlı** veri çekilir:

1. **Geocoding API** — şehir ismi verilir, enlem/boylam döner
2. **Forecast API** — koordinat verilir, o anki sıcaklık/rüzgar/nem döner

Her şehir için önce koordinat bulunur, sonra o koordinatla hava durumu sorgulanır; sonuçlar bir DataFrame'e toplanıp `hava_durumu.csv` olarak kaydedilir. Ülke verisinin aksine (nüfus, yüzölçümü gibi neredeyse hiç değişmeyen değerler), hava durumu her çalıştırmada farklı sonuç verir — bu da API kullanmanın CSV'ye kıyasla asıl değerini gösterir.

### 7. ETL Pipeline (`etl_pipeline.py`)
Extract, Transform ve Load adımlarının ayrı fonksiyonlara bölünüp tek bir akışta birleştirilmesi:

- **Extract** — geocoding + forecast API'lerinden şehir verileri çekilir; bir şehir bulunamazsa veya istek başarısız olursa `try/except` ile o şehir atlanır, pipeline çökmeden devam eder
- **Transform** — Celsius'tan Fahrenheit'a dönüşüm, sıcaklığa göre kategori (`Soğuk`/`Ilık`/`Sıcak`) atanması (`apply`), ve çekilme zamanının eklenmesi
- **Load** — sonuç PostgreSQL'deki `hava_durumu_log` tablosuna **eklenir** (`if_exists="append"`), önceki ölçümler silinmez

`if_exists="replace"` (sabit veri, her seferinde üzerine yazılır) ile `if_exists="append"` (zamanla biriken ölçüm/log verisi) arasındaki seçim, verinin doğasına göre yapılır — burada her çalıştırma yeni bir ölçüm olduğundan `append` kullanılır.

`if __name__ == "__main__":` bloğu sayesinde dosya hem doğrudan çalıştırılabilir hem de fonksiyonları başka bir dosyadan içe aktarılabilir.
...
- `requests` ile dış API'lere istek atma, JSON yanıtları çözme
- İki API'yi zincirleme (bir API'nin çıktısını diğerine girdi olarak verme)
- `apply` ile satır bazlı dönüşüm
- ETL deseni: Extract / Transform / Load adımlarının ayrıştırılması
- Hataya dayanıklı veri çekme (`try/except` ile kısmi başarısızlıkta pipeline'ın çökmemesi)
- `if_exists="replace"` ile `"append"` arasındaki fark ve doğru kullanım senaryoları